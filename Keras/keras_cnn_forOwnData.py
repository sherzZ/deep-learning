...
if num_channel==1:
    if K.image_dim_ordering()=='th':                #(808L,128L,128L)
        img_data= np.expand_dims(img_data, axis=1)  #(num of samples, channel, img_size,img_size) (808L,1L,128L,128L)
        print (img_data.shape)
    else:
        img_data= np.expand_dims(img_data, axis=4)  # tensorflow 将(808L,128L,128L)->(808L,128L,128L,1L)
        print (img_data.shape)
